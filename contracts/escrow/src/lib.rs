...
fn ensure_not_paused(env: &Env) {
    assert!(!storage::read_paused(env), "contract paused");
}
...
fn get_ttl_extension(env: &Env) -> u32 {
    storage::read_ttl_extension(env)
}
...
impl Escrow {
    ...
    pub fn initialize(env: Env, admin: Address, fee_collector: Address, arbitration_fee: i128) {
        ...
        storage::write_fee_collector(&env, &fee_collector);
        storage::write_arbitration_fee(&env, &arbitration_fee);
        storage::write_escrow_counter(&env, &1u64);
    ...
    pub fn set_admin(env: Env, new_admin: Address) {
        let old_admin = storage::read_admin(&env);
    ...
    pub fn get_fee_config(env: Env) -> FeeConfig {
        FeeConfig {
            collector: storage::read_fee_collector(&env),
