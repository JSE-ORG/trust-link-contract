//! Core escrow contract: the `Escrow` type, its `#[contractimpl]` entry points,
//! the state-transition matrix, and the internal storage/fee/validation helpers.
//!
//! Public items are re-exported from the crate root (`lib.rs`) so existing
//! `crate::*` / `super::*` paths keep resolving unchanged.
...
/// `ArithmeticOverflow`).
pub(crate) fn deduct_and_transfer(env: &Env, token_addr: &Address, recipient: &Address, amount: i128, fee_bps: u32) -> Result<(), ContractError> {
    if amount < 0 {
        ...
        if is_buyer {
            if let Some(buyer) = &escrow.buyer {
                token::Client::new(&env, &escrow.token)
                    .transfer(&env.current_contract_address(), buyer, &escrow.amount);
            }
            if escrow.fee_bps > 0 {
