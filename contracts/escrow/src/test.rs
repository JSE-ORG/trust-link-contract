...
use crate::{
    events::DisputePendingFinalization, AutoReleased, ContractError, DisputeRaised,
    DisputeResolved, Escrow, EscrowClient, EscrowCompleted, EscrowCreated, EscrowFunded,
    EscrowState, Payee, ResolutionType, ResolverSet,
};
use soroban_sdk::{
...
fn single_payee(env: &Env, address: &Address) -> Val {
...
fn test_raise_dispute_only_once() {
    ...
    let payees_66_val = payees_66.into_val(&env);
    let id = client.create_escrow(
        &payees_66_val,
        &None::<Address>,
        &resolver,
...
fn test_multiple_escrows() {
    ...
    let payees_67_val = payees_67.into_val(&env);
...
fn test_fund_and_confirm_delivery_with_non_usdc_token() {
    ...
    let payees_70_val = payees_70.into_val(&env);
    let id = client.create_escrow(
        &payees_70_val,
        &None::<Address>,
        &resolver,
...
fn test_dispute_resolved_to_seller_with_non_usdc_token() {
    ...
    let payees_71_val = payees_71.into_val(&env);
    let id = client.create_escrow(
        &payees_71_val,
        &None::<Address>,
        &resolver,
...
fn test_dispute_refunded_to_buyer_with_non_usdc_token() {
    ...
    let payees_72_val = payees_72.into_val(&env);
    let id = client.create_escrow(
        &payees_72_val,
        &None::<Address>,
        &resolver,
...
fn test_auto_release_with_non_usdc_token() {
    ...
    let payees_73_val = payees_73.into_val(&env);
    let id = client.create_escrow(
        &payees_73_val,
        &None::<Address>,
        &resolver,
...
fn test_multi_asset_concurrent_escrows_different_tokens() {
    ...
    let payees_74_val = payees_74.into_val(&env);
    let id1 = client.create_escrow(
        &payees_74_val,
        &None::<Address>,
        &resolver,
    ...
    let payees_75_val = payees_75.into_val(&env);
    let id2 = client.create_escrow(
        &payees_75_val,
        &None::<Address>,
        &resolver,
    ...
    env.ledger().set_timestamp(env.ledger().timestamp() + 86401);
    client.finalize_dispute(&resolver, &id2);

    let escrow1 = client.get_escrow(&id1);
    let escrow2 = client.get_escrow(&id2);
    assert_eq!(escrow1.state, EscrowState::Funded);
    assert_eq!(escrow2.state, EscrowState::Refunded);

    assert_eq!(get_balance(&env, &token_a, &seller), 0);
    assert_eq!(get_balance(&env, &token_a, &contract_id), 150);
...
fn test_zero_fee_no_collector_transfer() {
    ...
    let payees_76_val = payees_76.into_val(&env);
    let id = client.create_escrow(
        &payees_76_val,
        &None::<Address>,
        &resolver,
...
fn test_dispute_after_shipping_succeeds() {
    ...
    let payees_77_val = payees_77.into_val(&env);
    let id = client.create_escrow(
        &payees_77_val,
        &None::<Address>,
        &resolver,
...
fn test_dispute_requires_shipped_state() {
    ...
    let payees_78_val = payees_78.into_val(&env);
    let id = client.create_escrow(
        &payees_78_val,
        &None::<Address>,
        &resolver,
...
fn test_auto_release_after_dispute_deadline() {
    ...
    let payees_79_val = payees_79.into_val(&env);
    let id = client.create_escrow(
        &payees_79_val,
        &None::<Address>,
        &resolver,
...
fn test_fee_change_does_not_affect_funded_escrow() {
    ...
    let payees_80_val = payees_80.into_val(&env);
...
fn test_event_integrity_escrow_created() {
    ...
    let payees_81_val = payees_81.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_81_val,
        &None::<Address>,
        &resolver,
    ...
    assert!(has_event::<EscrowCreated, _>(&env, &cid, "Escrow", |ev| {
...
fn test_event_integrity_escrow_funded() {
    ...
    let payees_82_val = payees_82.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_82_val,
        &None::<Address>,
        &resolver,
    ...
    assert!(has_event::<EscrowFunded, _>(&env, &cid, "Escrow", |ev| {
        ev.escrow_id == escrow_id as u64 && ev.buyer == buyer && ev.amount == 500
    }));
}
...
fn test_event_integrity_escrow_completed_via_confirm_delivery() {
    ...
    let payees_83_val = payees_83.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_83_val,
        &None::<Address>,
        &resolver,
...
fn test_event_integrity_dispute_raised() {
    ...
    let payees_84_val = payees_84.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_84_val,
        &None::<Address>,
        &resolver,
...
fn test_event_integrity_dispute_resolved_release_to_seller() {
    ...
    let payees_85_val = payees_85.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_85_val,
        &None::<Address>,
        &resolver,
...
fn test_event_integrity_dispute_resolved_refund_buyer() {
    ...
    let payees_86_val = payees_86.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_86_val,
        &None::<Address>,
        &resolver,
...
fn test_event_integrity_auto_released() {
    ...
    let payees_87_val = payees_87.into_val(&env);
    let escrow_id = client.create_escrow(
        &payees_87_val,
        &None::<Address>,
        &resolver,
    ...
    assert!(has_event::<AutoReleased, _>(&env, &cid, "Escrow", |ev| {
        ev.escrow_id == escrow_id as u64
            && ev.seller == seller
            && ev.amount == 1000
            && ev.fee_bps == 200
...
fn test_event_integrity_full_lifecycle_all_events_decoded() {
    ...
    let payees_88_val = payees_88.into_val(&env);
    let id1 = client.create_escrow(
        &payees_88_val,
        &None::<Address>,
        &resolver,
    ...
    let payees_89_val = payees_89.into_val(&env);
    let id2 = client.create_escrow(
        &payees_89_val,
        &None::<Address>,
        &resolver,
    ...
    client.resolve_dispute(&resolver, &id2, &ResolutionType::Refund);
    assert!(has_event::<DisputePendingFinalization, _>(
        &env,
        &cid,
        "Dispute",
...
fn test_cancel_escrow_by_buyer_refunds_full_amount() {
    ...
    let payees_90_val = payees_90.into_val(&env);
    let id = client.create_escrow(
        &payees_90_val,
        &None::<Address>,
        &resolver,
...
fn test_cancel_escrow_state_transitions_correctly() {
    ...
    let payees_91_val = payees_91.into_val(&env);
    let id = client.create_escrow(
        &payees_91_val,
        &None::<Address>,
        &resolver,
...
fn test_cancel_escrow_pending_escrow_by_seller_succeeds() {
    ...
    let payees_92_val = payees_92.into_val(&env);
    let id = client.create_escrow(
        &payees_92_val,
        &None::<Address>,
        &resolver,
...
fn test_cancel_escrow_completed_escrow_fails() {
    ...
    let payees_93_val = payees_93.into_val(&env);
    let id = client.create_escrow(
        &payees_93_val,
        &None::<Address>,
        &resolver,
...
fn test_cancel_escrow_already_cancelled_fails() {
    ...
    let payees_94_val = payees_94.into_val(&env);
    let id = client.create_escrow(
        &payees_94_val,
        &None::<Address>,
        &resolver,
...
fn test_cancel_escrow_with_zero_fee() {
    ...
    let payees_95_val = payees_95.into_val(&env);
    let id = client.create_escrow(
        &payees_95_val,
        &None::<Address>,
        &resolver,
...
fn test_cancel_escrow_preserves_escrow_metadata() {
    ...
    let payees_96_val = payees_96.into_val(&env);
    let id = client.create_escrow(
        &payees_96_val,
        &None::<Address>,
        &resolver,
