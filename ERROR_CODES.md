...
| **4** | `InvalidState` | The escrow is in a lifecycle state that does not permit the requested action. A `Completed` or `Refunded` escrow returns `EscrowAlreadyCompleted` (49) / `EscrowAlreadyRefunded` (50) instead where the check specifically distinguishes those two terminal states. | Consult the state machine diagram to ensure the action is valid for the current state. |
...
| **16** | `InvalidStateTransition` | The requested state change is not allowed by the escrow lifecycle rules. A `Completed` or `Refunded` escrow returns `EscrowAlreadyCompleted` (49) / `EscrowAlreadyRefunded` (50) instead where the check specifically distinguishes those two terminal states. | Ensure the action follows the allowed sequence (e.g., Pending -> Funded -> Shipped). |
...
| **49** | `EscrowAlreadyCompleted` | The action requires a different state, but the escrow has already been released to the payees (`Completed`). Returned in place of `InvalidState` / `InvalidStateTransition` where the check specifically distinguishes this terminal state. | The escrow is fully settled; no further lifecycle actions apply. |
| **50** | `EscrowAlreadyRefunded` | The action requires a different state, but the escrow has already been refunded to the buyer (`Refunded`). Returned in place of `InvalidState` / `InvalidStateTransition` where the check specifically distinguishes this terminal state. | The escrow is fully settled; no further lifecycle actions apply. |
| **51** | `NotAuthorizedBuyer` | The caller is required to be the escrow's buyer, but is not (`fund_escrow`, `fund_basket_escrow`, `confirm_delivery`, `request_refund`, `raise_dispute`). Returned in place of `NotAuthorized` where the check specifically compares the caller against the buyer. | Call with the escrow's designated buyer address, or leave `buyer` unset at creation to allow any funder. |
| **52** | `NotAuthorizedSeller` | The caller is required to be one of the escrow's payees (sellers) — the primary payee for `approve_refund`, any payee for `mark_shipped` — but is not. Returned in place of `NotAuthorized` where the check specifically compares the caller against the payee list. | Call with an address from the escrow's `payees` list (or `payees[0]` where the primary payee specifically is required). |
